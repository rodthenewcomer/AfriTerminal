...
- Stock charts: all 48 web and native sheets use one calendar engine for 1D/1W/1M/3M/6M/YTD/1Y/3Y/5Y/MAX. Every selection recomputes exact dates, initial/final closes, price and annualized return, period high/low, volume, no-trade sessions, best/worst session, paid dividends and total return. If no intraday samples exist, 1D displays the last official session and measures it against the previous close instead of blocking the chart. Five years never aliases MAX. The publishing pipeline removes isolated PDF thousand-separator corruption, rejects invalid OHLCV and reports every persistent move above 50% for corporate-action review.
